...
from flexs.baselines.models import MLPEnsemble
...
def can_mutate_single_pos(sample, pos, mut_id, stop_token):
  if stop_token is None:
    return True

  if mut_id == stop_token:
    return False

  stop_idx = np.nonzero(sample == stop_token)
  return len(stop_idx[0]) == 0 or stop_idx[0][0] > np.nonzero(pos)[0][0]

def apply_mutations(samples, mutation_types, pos_masks, mutations,
                    use_assignment_mutations=False, stop_token=None):
  ...
  for i in range(num_mutations):
    ...
    if use_assignment_mutations:
      ...
      if stop_token:
        for j in range(len(perturbed_samples)):
          nonzero = np.nonzero(perturbed_samples[i] == stop_token)
          if len(nonzero[0]) == 0:
            continue

          perturbed_samples[i, nonzero[0][0]:] = stop_token
      ...
      for j in range(batch_size):
        ...
        pos = pos_masks[j, i]
        mut_id = int(mutation_types[j, i])

        mutate_single_pos = can_mutate_single_pos(sample, pos, mut_id, stop_token)
        perturbed_sample = sample
        if mutate_single_pos:
          mutation = mutations[mut_id]
          perturbed_sample = mutation(sample, pos)
        elif mut_id == stop_token:
          sample[pos:] = stop_token
          perturbed_sample = sample
...
def gen_rand_seq(min_seq_len, max_seq_len, num_possible_chars):
    embed = np.full((max_seq_len,), num_possible_chars)
    this_seq_len = np.random.randint(min_seq_len, max_seq_len + 1)

    chars = np.random.choice(num_possible_chars - 1, this_seq_len)
    embed[:len(chars)] = chars

    return embed

def init_population(min_seq_len, max_seq_len, alphabet, population_size):
    population = data.Population()

    is_fixed_length_task = min_seq_len == max_seq_len
    seqs = [
        gen_rand_seq(min_seq_len, max_seq_len, len(alphabet))
        for _ in range(population_size)
    ]

    samples = [data.Sample(structure=seq, batch_index=0) for seq in seqs]
    population.add_samples(samples)
...
@gin.configurable
class MutationPredictorSolver(flexs.Explorer, base_solver.BaseSolver):
  ...
  def __init__(self,
    ...
    """
    base_solver.BaseSolver.__init__(
        self, domain=domain, random_state=random_state, **kwargs)
    self.rng = jrand.PRNGKey(random_state)
    self.rng, rng = jax.random.split(self.rng)

    if num_mutations:
      self.cfg.num_mutations = num_mutations
    ...
    self.batch_size = sequences_batch_size
    self._initialized = True
    self.num_inner_rounds = num_model_rounds
    ...
    model = MLPEnsemble(seq_len, alphabet, acq_class)
    flexs.Explorer.__init__(
        self,
        model,
        "AmortizedBO",
        rounds,
        sequences_batch_size,
        self.num_inner_rounds * sequences_batch_size,
        starting_sequence,
        log_file,
    )
  ...
  def _update_population_after_oracle(self, measured_sequences_data):
    last_batch = self.population.get_last_batch().copy()
    for i in range(len(last_batch)):
      last_batch[i].structure = measured_sequences_data['sequence'][i]
      last_batch[i].reward = measured_sequences_data['true_score'][i]
      last_batch[i].batch_idx += 1

    population.add_samples(last_batch)

  def _is_seq_long_enough(self, seq):
    return len(seq) >= self.min_proposal_seq_len

  def _get_usable_seqs(self, samples, rewards, measured_seqs_data):
    measured_seqs = set(measured_seqs_data["sequence"])
    is_usable_seq = (
    ...
    to_propose = {
      ...
      if is_usable_seq(samples[i])
    }

    return np.array(list(to_propose.keys())), np.array(list(to_propose.values()))

  def cat_encoding_to_str(self, cat_encodings):
    stop_idx = -1 if not self.domain.is_dynamic_length else self.domain.vocab_size
    strs = []
    for cat_encoding in cat_encodings:
      chars = []
      for char_idx in cat_encoding:
        if char_idx == stop_idx:
          break
  ...
  def propose_sequences(self, measured_sequences_data, this_batch):
    #if this_batch is not None:
      # Calls to propose will update params from the last sequences,
      # so just add something to the grad_update guy here so that it
      # will update with the data screened against the oracle.
      #self._update_population_after_oracle(this_batch)
      #self._data_for_grad_update.append(self._last_grad_update_data)

    all_samples, all_rewards = [], []
    for batch_idx in range(self.num_inner_rounds):
      samples = self.propose(self.batch_size, self.population.copy())
      if not isinstance(samples[0], data.Sample):
        samples = [data.Sample(structure=structure) for structure in samples]

      sample_structures = [sample.structure for sample in samples]
      samples_as_str = self.cat_encoding_to_str(sample_structures)
      rewards = self.model.get_fitness(samples_as_str, compute_acq=True)

      all_samples.append(samples_as_str)
    ...
    usable_seqs, usable_rewards = self._get_usable_seqs(
      all_samples,
      all_rewards,
      measured_sequences_data,
    )

    sorted_order = np.argsort(usable_rewards)[::-1][: self.sequences_batch_size]
    return usable_seqs[sorted_order], usable_rewards[sorted_order]
  ...
  def _apply_mutations(self, parents, mutation_types, pos_masks):
    return apply_mutations(parents, mutation_types, pos_masks, self.mutations,
  ...
  def _perturb(self, parents, mode="train"):
    ...
    states = self._apply_mutations(parents, mutation_types, pos_masks)
  ...
  def _update_params(self, population):
    ...
    self._last_grad_update_data = dat
...
@gin.configurable
class MutationPredictorSolverDynamicLength(MutationPredictorSolver):
  def _apply_mutations(self, parents, mutation_types, pos_masks):
    assert self.domain.is_dynamic_length
    return apply_mutations(parents, mutation_types, pos_masks, self.mutations,
                           use_assignment_mutations=True, stop_token=self.domain.vocab_size)
